...
lt_slim <- lt_ae %>% 
  select(species:wt_g, sex, age_est, cr)
...
for (i in 1:length(smp)) {
  ...
  vb_confit <- Confint(f.fit) %>%
    ...
    as_tibble()
  vb_output$crew <- crews
  vb_confit$crew <- crews
  ...
  preds1 <- data.frame(crews,
                       a,
                       predict(f.fit, data.frame(age_est = a)),
                       Confint(f.boot2)) 
  
  
  names(preds1) <- c("crew", "age", "fit", "LCI", "UCI")
