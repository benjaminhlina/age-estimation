...
jags_model <- function() {
  ...
  for (i in 1:n_pred){
    ...
    pred_length[i, 2] <- (linf + linf_diff) * (1 - exp(-k *(pred_age[i] - t0)))
    pred_length[i, 3] <- (linf - linf_diff) * (1 - exp(-k *(pred_age[i] - t0)))
...
jags_inits <- function(nc) {
  ...
  for (c in 1:nc) {
    inits[[c]] = list(
      ...
      linf_diff = runif(1, -50, 50), # random value between -20 and 20 to start
      # differences 
...
diag_p <- c("linf", "k", "t0", "sig", "linf_diff")
...
labels <- paste("TL ==", linf,"~group('(',1-e^{-", k,"~(age", t0,")},')')")
...
ggsave(filename = here("Plots",
                       "von B Bayes",
                       "growth_curve_lt_bayes_basin.png"), 
...
ggsave(filename = here("Plots", 
                       ...
                       "predicted_density_lkt_age_length_basin_fill.png"), 
...
ggsave(filename = here("Plots",
                       "posterior distribution",
                       "predicted_density_ridge_tornado_age_length_basin.png"), 
